...
export default function Dashboard() {
  ...
  const fetchDashboardPollerServices = useCallback(
    async (pollerId, type = "all", page = 1, limit = 100, statusFilterParam = "unhandled") => {
      if (!pollerId) return;
      ...
      try {
        setIsLoadingDashboardGlobalList(true);
        setDashboardGlobalServices([]);
        setDashboardGlobalMeta({ page, limit, total: 0, totalPages: 1 });
        ...
        const normalizedResults = (payload?.data?.result || []).map(normalizeService);

        setDashboardGlobalServices(normalizedResults);
        setDashboardGlobalMeta(
          payload?.meta || {
            page,
            limit,
        ...
        if (!isLatestRequest()) return;
        console.error("Error fetching dashboard poller services:", error);
        setDashboardGlobalServices([]);
        setDashboardGlobalMeta({ page, limit, total: 0, totalPages: 1 });
      } finally {
        if (isLatestRequest()) {
  ...
  const fetchPollerServiceSummary = useCallback(
    async ({
      pollerId,
      type = "all",
      page = 1,
      ...
      try {
        ...
        const normalizedResults = (payload?.data?.result || []).map(
          normalizeService,
        );
  ...
  const dashboardPollerMode = useMemo(
    () => location.pathname === "/dashboard" && filters.poller !== "all",
    [location.pathname, filters.poller],
  );

  const selectedDashboardPollerId = useMemo(() => {
    if (!dashboardPollerMode) return null;
    const match = cachedPollers.find((p) => p.Poller === filters.poller);
    return match?.poller_id ?? null;
  }, [dashboardPollerMode, cachedPollers, filters.poller]);
  ...
  useEffect(() => {
    if (dashboardPollerMode && selectedDashboardPollerId) {
      fetchDashboardPollerServices(
        selectedDashboardPollerId,
        currentTableType,
        servicePage,
        serviceLimit,
        statusFilter,
  ...
  const pollerDropdownList = useMemo(
    () => cachedPollers.map((p) => p.Poller).filter(Boolean),
    [cachedPollers],
  );
  ...
  const dashboardTableServices = useMemo(() => {
    const source =
      dashboardGlobalListMode || dashboardPollerMode
                  ...
                  <div className="pollers-header">
                    ...
                    <div className="pollers-pagination-controls">
                      <div className="pollers-pagination-left">
                        <span className="pollers-pagination-info">
                          Service Page{" "}
                          {pollerServiceMeta.page || pollerServicePage} of{" "}
                          {pollerServiceMeta.totalPages || 1}
                          {" | "} Hosts: {pollerHostMeta.total || 0}
                          {" | "} Matching Services:{" "}
                          {pollerServiceMeta.total || 0}
                        </span>
                      </div>
                      <div className="pollers-pagination-right">
                        ...
                        <div className="pollers-pagination-buttons">
                          <button
                            className="pollers-page-btn"
                            onClick={() => {
                              setPollerServicePage((prev) =>
                                Math.max(prev - 1, 1),
                              );
                            }}
                            disabled={
                              pollerServicePage <= 1 || isLoadingPollerServices
                            }
                          >
                            ◀ Prev
                          </button>
                          <span className="pollers-page-info">
                            Page {pollerServiceMeta.page || pollerServicePage}{" "}
                            of {pollerServiceMeta.totalPages || 1}
                          </span>
                          <button
                            className="pollers-page-btn"
                            onClick={() => {
                    ...
                    <div className="filter-controls-inline">
                      <div className="filter-input-group-compact">
                        ...
                        <FilterCombobox
                          ...
                          onChange={(value) => {
                            setPollerServiceFilters((current) => ({
                              ...current,
                              host: value,
                            }));
                            setPollerServicePage(1);
                    ...
                    <table className="services-table">
                      ...
                      <tbody>
                        {isLoadingPollerServices ? (
                          <tr>
                            <td colSpan="5" className="loading-cell">
                              Loading authoritative unhandled services for{" "}
                              {selectedPoller}...
                            </td>
                          </tr>
                          ...
                          <tr>
                            <td colSpan="5" className="loading-cell">
                              No unhandled Critical, Warning, or Unknown
                              services match the selected Poller filters.
                            </td>
                          </tr>
                        ) : (
